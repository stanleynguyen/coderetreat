...
class Game {
  ...
  next() {
    this.cellList = this.cellList.map((c, i) =>
      this.calNextState(c, this.calAliveMates(...this[calXY](i))),
    );
  }
  ...
  calAliveMates(x, y) {
    return this.getMatesIndices(x, y).reduce(
      (curr, val) => curr + this.getCell(...val),
      0,
    );
  }
  ...
  isSkipCell(xIndex, yIndex, x, y) {
    return (
      this[isIndexOutOfBounds](xIndex, yIndex) ||
      this[isSameCell](xIndex, yIndex, x, y)
    );
  }
  ...
  display() {
    return this.cellList
      .map((cell, index) => {
        let cellString = this.isDead(cell)
          ? CELL_STRINGS.DEAD
          : CELL_STRINGS.ALIVE;
